        ...
        {!user ? (
          ...
          <div className="flex-shrink-0 w-52 space-y-3 sticky top-24">
            <div className="bg-slate-900/40 border border-white/5 rounded-xl p-3 space-y-2">
              {[1, 2, 3].map(i => (
                <button key={i} onClick={() => i <= step && setStep(i)} className={`flex items-center gap-3 w-full px-10 py-4 rounded-xl border transition-all ${stepClass(i)}`}>
            ...
            {step === 2 && weatherLoaded && <div className="bg-slate-900/40 border border-white/5 rounded-xl p-3 space-y-2">
              {ELEVATION_PRESETS.map(p => (
                <button key={p.label} onClick={()=>applyPreset(p.label)} className={`w-full text-left px-10 py-2 rounded-xl border transition-all text-xs ${selectedPreset===p.label?'border-orange-500 bg-orange-500/10 text-white':'border-white/10 bg-slate-900/60 text-slate-400 hover:border-orange-500/50'}`}>
