        ...
        {!user ? (
          ...
          <div className="space-y-6">
            ...
            {step === 2 && <div className="fade-in space-y-4">
              ...
              {weatherLoaded && <>
                <div className="flex justify-center"><div className="grid grid-cols-3 gap-4 w-full max-w-2xl">{ELEVATION_PRESETS.map(p => <button key={p.label} onClick={()=>applyPreset(p.label)} className={`text-center px-5 py-4 rounded-2xl border transition-all text-sm ${selectedPreset===p.label?'border-orange-500 bg-orange-500/10 text-white shadow-lg shadow-orange-500/10':'border-white/10 bg-slate-900/60 text-slate-400 hover:border-orange-500/50'}`}><span className="text-2xl mr-2">{p.icon}</span><span className="font-bold text-base block mb-1">{p.label}</span><span className="text-xs text-slate-500">{p.deltaTemp>0?'+':''}{p.deltaTemp}°C · {p.deltaHumidity>0?'+':''}{p.deltaHumidity}% HR</span></button>)}</div></div>
              </>}
