            ...
            {step === 2 && <div className="fade-in space-y-4">
              ...
              {weatherLoaded && <div className="bg-slate-900/60 border border-white/5 rounded-xl p-5">
                <div className="flex items-center flex-wrap gap-4 md:gap-6">
                  <div className="text-center flex-1 min-w-[100px]"><p className="text-[9px] text-slate-500 uppercase tracking-widest font-bold mb-1.5">Temperatura</p><div className="flex items-baseline justify-center gap-2"><span className="text-5xl md:text-6xl font-black text-orange-500">{effectiveTemp}</span><span className="text-xl text-orange-500/70 font-black">°C</span></div></div>
                  {adjustTemp!==0 && <div className="text-center flex-1 min-w-[80px]"><p className="text-[9px] text-slate-500 uppercase tracking-widest font-bold mb-1.5">Base</p><div className="flex items-baseline justify-center gap-2"><span className="text-5xl md:text-6xl font-black text-white">{baseTemp}</span><span className="text-xl text-white/70 font-black">°C</span></div></div>}
                  <div className="w-px h-24 bg-white/5 flex-shrink-0" />
                  <div className="text-center flex-1 min-w-[100px]"><p className="text-[9px] text-slate-500 uppercase tracking-widest font-bold mb-1.5">Humedad</p><div className="flex items-baseline justify-center gap-2"><span className="text-5xl md:text-6xl font-black text-blue-400">{effectiveHumidity}</span><span className="text-xl text-blue-400/70 font-black">%</span></div></div>
                  {adjustHumidity!==0 && <div className="text-center flex-1 min-w-[80px]"><p className="text-[9px] text-slate-500 uppercase tracking-widest font-bold mb-1.5">Base</p><div className="flex items-baseline justify-center gap-2"><span className="text-5xl md:text-6xl font-black text-white">{baseHumidity}</span><span className="text-xl text-white/70 font-black">%</span></div></div>}
              ...
              {weatherLoaded && <>
                <div className="flex justify-center"><div className="grid grid-cols-3 gap-3 w-full max-w-xl">{ELEVATION_PRESETS.map(p => <button key={p.label} onClick={()=>applyPreset(p.label)} className={`text-center px-4 py-3 rounded-xl border transition-all text-xs ${selectedPreset===p.label?'border-orange-500 bg-orange-500/10 text-white':'border-white/10 bg-slate-900/60 text-slate-400 hover:border-orange-500/50'}`}><span className="text-xl mr-1">{p.icon}</span><span className="font-bold text-sm block mb-0.5">{p.label}</span><span className="text-[10px] text-slate-500">{p.deltaTemp>0?'+':''}{p.deltaTemp}°C · {p.deltaHumidity>0?'+':''}{p.deltaHumidity}% HR</span></button>)}</div></div>
              </>}
