...
export default function AlertaPresionPage() {
  ...
  const fetchWeather = async (lat: number, lng: number, altitude?: number) => {
    setWeatherLoading(true); setError(''); setAltitudeAdjusted(false); setProfileExpanded(false);
  ...
  useEffect(() => { if (recommendation && resultsRef.current) resultsRef.current.scrollIntoView({ behavior: 'smooth', block: 'start' }); }, [recommendation]);
          ...
          <>
            ...
            <details open={profileExpanded} className="bg-slate-900/40 border border-white/5 rounded-2xl">
              <summary className="flex items-center gap-2 p-6 cursor-pointer text-slate-400 hover:text-white transition-colors list-none">
                ...
                <svg className={`w-3 h-3 text-slate-600 transition-transform ${profileExpanded ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
              </summary>
              <div className="px-6 pb-6 space-y-4">
                {profileLoadError && <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-lg text-[11px] text-red-400">Error al cargar perfiles: {profileLoadError}</div>}
                ...
                {savedProfiles.length === 0 && profileLoaded && !profileLoadError && <div className="text-[11px] text-slate-500 bg-slate-800/20 rounded-lg px-4 py-3">No tienes perfiles guardados. Configura y guarda tu perfil para usarlo después.</div>}
                <div className="flex items-center gap-3">
                  ...
                  {profileLoaded && <button onClick={handleSaveProfile} disabled={saving} className={`mt-5 px-4 py-2 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-colors whitespace-nowrap ${saveStatus === 'saved' ? 'bg-green-500/20 text-green-400 border border-green-500/30' : selectedProfileId ? 'bg-blue-500/10 text-blue-400 border border-blue-500/25 hover:bg-blue-500/20' : 'bg-orange-500/10 text-orange-400 border border-orange-500/25 hover:bg-orange-500/20'}`}>{saving ? 'Guardando...' : saveStatus === 'saved' ? '✓ Guardado' : selectedProfileId ? 'Actualizar' : 'Guardar nuevo'}</button>}
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div><label className="text-[10px] text-slate-500 uppercase tracking-widest font-bold block mb-1">Peso ciclista (kg)</label>
                    <select value={profile.riderWeightKg} onChange={e => setProfile(p => ({ ...p, riderWeightKg: Number(e.target.value) }))} className="w-full bg-slate-950 border border-white/5 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-orange-500/40 appearance-none cursor-pointer">{Array.from({ length: 33 }, (_, i) => 68 + i).map(o => <option key={o} value={o}>{o}</option>)}</select></div>
                  <div><label className="text-[10px] text-slate-500 uppercase tracking-widest font-bold block mb-1">Peso bicicleta (kg)</label>
                    <select value={profile.bikeWeightKg} onChange={e => setProfile(p => ({ ...p, bikeWeightKg: Number(e.target.value) }))} className="w-full bg-slate-950 border border-white/5 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-orange-500/40 appearance-none cursor-pointer">{Array.from({ length: 15 }, (_, i) => 11 + i).map(o => <option key={o} value={o}>{o}</option>)}</select></div>
                  <div><label className="text-[10px] text-slate-500 uppercase tracking-widest font-bold block mb-1">Modelo bicicleta</label>
                    <select value={profile.bikeModel} onChange={e => { const model = e.target.value; const spec = BIKE_MODELS.find(m => m.name === model); if (spec) setProfile({ ...profile, bikeModel: model, bikeWeightKg: spec.weightKg, wheelFront: spec.wheelFront, wheelRear: spec.wheelRear, tireWidthFrontInch: spec.tireWidthFrontInch, tireWidthRearInch: spec.tireWidthRearInch, tireModelFront: spec.tireModelFront, tireModelRear: spec.tireModelRear, tubeless: spec.tubeless }); else setProfile(p => ({ ...p, bikeModel: model })); }} className="w-full bg-slate-950 border border-white/5 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-orange-500/40 appearance-none cursor-pointer">
                  ...
                  <div><label className="text-[10px] text-slate-500 uppercase tracking-widest font-bold block mb-1">Rueda delantera</label>
                    <select value={profile.wheelFront} onChange={e => setProfile(p => ({ ...p, wheelFront: e.target.value as any }))} className="w-full bg-slate-950 border border-white/5 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-orange-500/40 appearance-none cursor-pointer"><option value="29">29"</option><option value="27.5">27.5"</option></select></div>
                  <div><label className="text-[10px] text-slate-500 uppercase tracking-widest font-bold block mb-1">Rueda trasera</label>
                    <select value={profile.wheelRear} onChange={e => setProfile(p => ({ ...p, wheelRear: e.target.value as any }))} className="w-full bg-slate-950 border border-white/5 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-orange-500/40 appearance-none cursor-pointer"><option value="29">29"</option><option value="27.5">27.5"</option></select></div>
                  <div><label className="text-[10px] text-slate-500 uppercase tracking-widest font-bold block mb-1">Ancho neumático del. (")</label>
                    <select value={profile.tireWidthFrontInch} onChange={e => setProfile(p => ({ ...p, tireWidthFrontInch: Number(e.target.value) }))} className="w-full bg-slate-950 border border-white/5 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-orange-500/40 appearance-none cursor-pointer">{[2.0, 2.1, 2.2, 2.3, 2.4, 2.5, 2.6, 2.7, 2.8].map(v => <option key={v} value={v}>{v.toFixed(1)}"</option>)}</select></div>
                  <div><label className="text-[10px] text-slate-500 uppercase tracking-widest font-bold block mb-1">Ancho neumático tras. (")</label>
                    <select value={profile.tireWidthRearInch} onChange={e => setProfile(p => ({ ...p, tireWidthRearInch: Number(e.target.value) }))} className="w-full bg-slate-950 border border-white/5 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-orange-500/40 appearance-none cursor-pointer">{[2.0, 2.1, 2.2, 2.3, 2.4, 2.5, 2.6, 2.7, 2.8].map(v => <option key={v} value={v}>{v.toFixed(1)}"</option>)}</select></div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div><label className="text-[10px] text-slate-500 uppercase tracking-widest font-bold block mb-1">Ancho llanta (mm int.)</label>
                    <select value={profile.rimWidthMm || 30} onChange={e => setProfile(p => ({ ...p, rimWidthMm: Number(e.target.value) }))} className="w-full bg-slate-950 border border-white/5 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-orange-500/40 appearance-none cursor-pointer">{[25, 27, 30, 32, 35, 38, 40].map(v => <option key={v} value={v}>{v}mm</option>)}</select></div>
                  ...
                  <div><label className="text-[10px] text-slate-500 uppercase tracking-widest font-bold block mb-1">Condición del terreno</label>
                    <select value={profile.groundCondition || 'mixto'} onChange={e => setProfile(p => ({ ...p, groundCondition: e.target.value as any }))} className="w-full bg-slate-950 border border-white/5 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-orange-500/40 appearance-none cursor-pointer"><option value="humedo">Húmedo</option><option value="mixto">Mixto</option><option value="seco">Seco</option></select></div>
                  <div><label className="text-[10px] text-slate-500 uppercase tracking-widest font-bold block mb-1">Estilo de conducción</label>
                    <select value={profile.ridingStyle || 'moderado'} onChange={e => setProfile(p => ({ ...p, ridingStyle: e.target.value as any }))} className="w-full bg-slate-950 border border-white/5 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-orange-500/40 appearance-none cursor-pointer"><option value="conservador">Conservador</option><option value="moderado">Moderado</option><option value="agresivo">Agresivo</option></select></div>
                  <div><label className="text-[10px] text-slate-500 uppercase tracking-widest font-bold block mb-1">Experiencia del rider</label>
                    <select value={profile.riderExperience || 'intermedio'} onChange={e => setProfile(p => ({ ...p, riderExperience: e.target.value as any }))} className="w-full bg-slate-950 border border-white/5 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-orange-500/40 appearance-none cursor-pointer"><option value="principiante">Principiante</option><option value="intermedio">Intermedio</option><option value="avanzado">Avanzado</option><option value="experto">Experto</option></select></div>
                  <div><label className="text-[10px] text-slate-500 uppercase tracking-widest font-bold block mb-1">Tipo de carcasa</label>
                    <select value={profile.casingType || 'estandar'} onChange={e => setProfile(p => ({ ...p, casingType: e.target.value as any }))} className="w-full bg-slate-950 border border-white/5 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-orange-500/40 appearance-none cursor-pointer"><option value="ligera">Ligera</option><option value="estandar">Estándar</option><option value="reforzada">Reforzada</option></select></div>
                </div>
              </div>
            </details>
                ...
                <div className="mt-4">
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="space-y-3">
                      <p className="text-[10px] text-orange-400 uppercase tracking-widest font-bold flex items-center gap-2"><Crosshair className="w-3.5 h-3.5" /> Haz clic — datos reales de estaciones AEMET con altitud</p>
                      <div className="relative w-full h-[400px] lg:h-[550px] rounded-xl overflow-hidden border border-white/5">
                        <Map
                          ...
                          onClick={(e: MapMouseEvent) => { const map = e.target; let alt; try { alt = map.queryTerrainElevation?.(e.lngLat) ?? undefined; } catch {} fetchWeather(e.lngLat.lat, e.lngLat.lng, alt); }}
                      ...
                      <p className="text-[9px] text-slate-600 leading-relaxed">
                        {stationMarkers.length > 0 ? `Se muestran ${stationMarkers.length} estaciones AEMET con su temperatura real.` : 'Haz clic en el mapa para obtener datos de las estaciones AEMET cercanas con ajuste por altitud.'}
                    ...
                    <div className="space-y-4">
                      ...
                      {!weatherLoaded && !weatherLoading && <div className="text-[11px] text-slate-500 bg-slate-800/30 rounded-xl px-4 py-6 text-center">Haz clic en el mapa 3D para obtener los datos base de AEMET.</div>}
                      {weatherLoading && <div className="flex items-center gap-2 text-[11px] text-slate-400 bg-slate-800/30 rounded-xl px-4 py-6"><Loader2 className="w-4 h-4 animate-spin" /> Consultando estaciones AEMET...</div>}
                      {weatherLoaded && <>
                        <div className="bg-slate-900/60 border border-white/5 rounded-xl p-5">
                          <div className="flex items-center justify-between mb-4"><span className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Datos actuales</span><span className="text-xs text-slate-400 truncate max-w-[180px]">{weatherSource}</span></div>
                        ...
                        <div><p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold mb-2">Perfiles rápidos</p>
                          <div className="grid grid-cols-2 gap-2">
                            {ELEVATION_PRESETS.map(p => (
                              <button key={p.label} onClick={() => applyPreset(p.label)} className={`text-left px-3 py-2 rounded-xl border transition-all text-[10px] ${selectedPreset === p.label ? 'border-orange-500 bg-orange-500/10 text-white' : 'border-white/10 bg-slate-900/60 text-slate-400 hover:border-orange-500/50'}`}>
                                <span className="text-xs mr-1">{p.icon}</span><span className="font-bold">{p.label}</span>
                                <span className="block text-[8px] text-slate-500 mt-0.5">{p.deltaTemp > 0 ? '+' : ''}{p.deltaTemp}°C · {p.deltaHumidity > 0 ? '+' : ''}{p.deltaHumidity}% HR</span>
                              </button>
                            ))}
                          </div>
                        </div>
                    ...
                    <div className="relative mx-auto">
                      <div className="grid grid-cols-1 md:grid-cols-[1fr_2fr_1fr] items-center gap-4 md:gap-6">
                        ...
                        <div className="relative">
                          <img src="/102350-Cube-Stereo-Hybrid-ONE44-HPC-Race-800-blackline-2026-EBike-Fully-Mountainbike-00.jpg" alt="Cube Stereo Hybrid e-MTB" className="w-full h-auto" style={{ mixBlendMode: 'multiply' }} />
                        </div>
                        <div className="bg-slate-950/90 border border-orange-500/40 rounded-2xl px-6 py-5 text-center shadow-xl shadow-orange-500/10">
                    ...
                    <div className="border border-white/5 rounded-2xl p-4 bg-slate-900/30">
                      ...
                      {clickAltitude != null && (
                        <div className="mt-2 flex items-center gap-1.5 text-[9px] text-slate-500 bg-slate-950/30 rounded-lg px-2.5 py-1.5">
                          <span>🏔️</span><span>Altitud del punto: <strong className="text-slate-300">{Math.round(clickAltitude)} m</strong></span>
